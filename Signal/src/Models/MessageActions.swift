...
struct MessageActionBuilder {
    ...
    static func pinMessage(conversationViewItem: ConversationViewItem, delegate: MessageActionsDelegate) -> MessageAction {
        let label = "PIN MESSAGE"
        return MessageAction(.pin,
                             accessibilityLabel: label,
                             accessibilityIdentifier: UIView.accessibilityIdentifier(containerName: "message_action", name: "select_message"),
                             block: { [weak delegate] (_) in
                                delegate?.messageActionsPinMessage(conversationViewItem)
        })
    }
}
...
class ConversationViewItemActions: NSObject {
    ...
    class func textActions(conversationViewItem: ConversationViewItem, shouldAllowReply: Bool, delegate: MessageActionsDelegate) -> [MessageAction] {
        ...
        //reply -> forward -> pin -> copy -> delete -> info-menu
        ...
        if conversationViewItem.isGroupThread,
           let msg = conversationViewItem.interaction as? TSMessage,
           msg.wasRemotelyDeleted == false {
            let pinAction = MessageActionBuilder.pinMessage(conversationViewItem: conversationViewItem, delegate: delegate)
