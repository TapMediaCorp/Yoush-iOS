...
class ThreadMapping: NSObject {
    ...
    @objc(threadForUniqueId:)
    func thread(uniqueId: NSString) -> TSThread {
//        assert(indexPath.item <= threads.count)
//        return threads[indexPath.item]
        for t in threads {
            if(t.uniqueId == uniqueId as String) {
                return t
            }
        }
        let newThread = TSThread()
        return newThread
    }
...
//            print("thread hided: " + String(thread.isHided))
            if(!thread.isHided){
