...
public class UserNotificationActionHandler: NSObject {
    ...
    func handleNotificationResponse( _ response: UNNotificationResponse) throws -> Promise<Void> {
        ...
        print("UNNotificationContent \(response.notification.request.content)")
        
        switch response.actionIdentifier {
        case UNNotificationDefaultActionIdentifier:
            Logger.debug("default action")
            let key = AppNotificationUserInfoKey.threadId
            if let threadId = userInfo[key] as? String,
               threadId.count > 0 {
                var thread:TSThread?
                SDSDatabaseStorage.shared.uiRead { trans in
                    thread = TSThread.anyFetch(uniqueId: threadId, transaction: trans)
                }
                if thread != nil,
                   thread!.isHided == true {
                    Logger.debug("Ignore hided thread")
