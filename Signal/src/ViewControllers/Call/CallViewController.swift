...
private let REMAINING_INTERVAL = 40
...
class CallViewController: OWSViewController, CallObserver, CallServiceObserver, CallAudioServiceDelegate {
    ...
    override func loadView() {
        self.view = UIView()
        self.view.backgroundColor = UIColor.ows_accentBlueColorBackgroundCallNew
        self.view.layoutMargins = UIEdgeInsets(top: 30, left: 20, bottom: 16, right: 20)
    ...
    override func viewDidLoad() {
        ...
        if let groupCall = groupCall {
            DATABASE_STORE.uiRead { trans in
                self.contactNameLabel.text = self.contactsManager.displayName(for: groupCall.groupThread, transaction: trans)
            }
        }else {
...
//        AppEnvironment.shared.callService.addObserverAndSyncState(observer: self)
        ...
        if call.direction == .incoming {
            callWaitingTimer = WeakTimer.scheduledTimer(timeInterval: TimeInterval(REMAINING_INTERVAL),
                ...
                if self.call.state == .localRinging {
                    //If does not accept the call in 40s -> decline the call
                    self.didPressDeclineCall(sender: self.declineIncomingButton)
                }
            }
        }
    }
    ...
    func createContactViews() {
        ...
        if let _ = groupCall {
            //Does not create leave call view for group
    ...
    func createOngoingCallControls() {
        ...
        audioSourceButton = createButton(image: #imageLiteral(resourceName: "11-audio-call-speaker-inactive"),
        ...
        hangUpButton = createButton(image: #imageLiteral(resourceName: "13-hangup-active-wide-new"),
        ...
        audioModeMuteButton = createButton(image: #imageLiteral(resourceName: "14-audio-call-mute-inactive"),
                                           action: #selector(didPressMute))
        audioModeMuteButton.setImage(#imageLiteral(resourceName: "15-audio-call-mute-active"), for: .selected)
        ...
        audioModeVideoButton = createButton(image: #imageLiteral(resourceName: "16-audio-call-video-inactive"),
                                            action: #selector(didPressVideo))
        audioModeVideoButton.setImage(#imageLiteral(resourceName: "17-audio-call-video-active"), for: .selected)
        ...
        videoModeMuteButton = createButton(image: #imageLiteral(resourceName: "18-video-mute-unselected"),
                                           action: #selector(didPressMute))
        videoModeMuteButton.setImage(#imageLiteral(resourceName: "14-audio-call-mute-inactive"), for: .selected)
        ...
        videoModeFlipCameraButton = createButton(image: #imageLiteral(resourceName: "19-video-switch-camera-unselected"),
        ...
        videoModeVideoButton = createButton(image: #imageLiteral(resourceName: "20-video-video-unselected"),
                                            action: #selector(didPressVideo))
        videoModeVideoButton.setImage(#imageLiteral(resourceName: "21-video-video-selected"), for: .selected)
    ...
    func updateAvatarImage() {
        if let groupCall = call as? JitsiGroupCall,
           let groupThread = groupCall.groupThread {
            contactAvatarView.image = OWSAvatarBuilder.buildImage(thread: groupThread, diameter: 130)
        }
        else {
            contactAvatarView.image = OWSAvatarBuilder.buildImage(thread: thread, diameter: 130)
        }
    }

    func createIncomingCallControls() {

        acceptIncomingButton = createButton(image: #imageLiteral(resourceName: "10-call-active-wide"),
        ...
        declineIncomingButton = createButton(image: #imageLiteral(resourceName: "9-hangup-active-wide"),
        ...
        incomingCallControls = UIStackView(arrangedSubviews: [declineIncomingButton , acceptIncomingButton ])
    ...
    func updateCallUI(callState: CallState) {
        ...
        //Does not allow leave call view
        leaveCallViewButton.isHidden = true
        ...
        if self.hasAlternateAudioSources {
            ...
            audioSourceButton.setImage(#imageLiteral(resourceName: "11-audio-call-speaker-inactive"), for: .normal)
            audioSourceButton.setImage(#imageLiteral(resourceName: "12-audio-call-speaker-active"), for: .selected)
...
//        }
    ...
    @objc func didPressDeclineCall(sender: UIButton) {
        ...
        actionBlock?((self, "decline", call))
//        if let groupCall = groupCall {
//            actionBlock?((self, "decline", groupCall))
//        }else {
//            callUIAdapter.localHangupCall(call)
//
//            dismissIfPossible(shouldDelay: false)
//        }
    ...
    func dismissIfPossible(shouldDelay: Bool, completion: (() -> Void)? = nil) {
        callUIAdapter.audioService.delegate = nil
