...
@property (weak, nonatomic) UIImageView *wallpaperBackgroundView;
...
- (void)addNotificationListeners
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(themeDidChange:)
                                               ...
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(conversationWallPaperDidChange:)
                                                 name:OWSConversationWallPaperDidChange
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(groupCallStateDidChange:)
                                                 name:OWSGroupCallStateDidChange
                                               object:nil];
}
...
- (void)conversationWallPaperDidChange:(NSNotification *)notification
{
    dispatch_async(dispatch_get_main_queue(), ^{
        NSDictionary *userInfo = notification.userInfo;
        if ([userInfo[@"action"] isKindOfClass:NSString.class] &&
            [userInfo[@"thread"] isKindOfClass:TSThread.class]) {
            TSThread *thread = userInfo[@"thread"];
            NSString *action = userInfo[@"action"];
            BOOL shouldHandle = false;
            if ([self.thread isKindOfClass:TSContactThread.class] &&
                [thread isKindOfClass:TSContactThread.class]) {
                TSContactThread *th1 = (id)self.thread;
                TSContactThread *th2 = (id)thread;
                if (th1.contactPhoneNumber &&
                    th2.contactPhoneNumber &&
                    [th1.contactPhoneNumber isEqualToString:th2.contactPhoneNumber]) {
                    shouldHandle = true;
                }
            }
            else if ([self.thread isKindOfClass:TSGroupThread.class] &&
                     [thread isKindOfClass:TSGroupThread.class]) {
                TSGroupThread *th1 = (id)self.thread;
                TSGroupThread *th2 = (id)thread;
                if (th1.uniqueId &&
                    th2.uniqueId &&
                    [th1.uniqueId isEqualToString:th2.uniqueId]) {
                    shouldHandle = true;
                }
            }
            if (shouldHandle) {
                if ([action isEqualToString:@"set"]) {
                    TSOutgoingMessage *outgoingMessage = [self.conversationViewModel setWallPaper:userInfo];
                    if (outgoingMessage) {
                        [self messageWasSent:outgoingMessage];
                    }
                }
                else if ([action isEqualToString:@"remove"]) {
                    TSOutgoingMessage *outgoingMessage = [self.conversationViewModel removeWallPaper:userInfo];
                    if (outgoingMessage) {
                        [self messageWasSent:outgoingMessage];
                    }
                }
...
- (void)groupCallStateDidChange:(NSNotification *)notification
{
    dispatch_async(dispatch_get_main_queue(), ^{
        NSDictionary *userInfo = notification.userInfo;
        if ([userInfo[@"threadId"] isKindOfClass:NSString.class]) {
            NSString *threadId = userInfo[@"threadId"];
            BOOL hasCallInProgress = [userInfo[@"hasCallInProgress"] boolValue];
            BOOL shouldHandle = false;
            if ([self.thread isKindOfClass:TSGroupThread.class] &&
                [self.thread.uniqueId isEqualToString:threadId]) {
                shouldHandle = true;
            }
            if (shouldHandle) {
                [self updateBarButtonItems];
...
- (void)viewDidLoad
{
    ...
    UIImageView *wallpaperBackgroundView = [[UIImageView alloc] initWithFrame:self.view.bounds];
    wallpaperBackgroundView.contentMode = UIViewContentModeScaleAspectFill;
    wallpaperBackgroundView.clipsToBounds = true;
    [self.view insertSubview:wallpaperBackgroundView belowSubview:self.collectionView];
    [wallpaperBackgroundView autoPinEdgesToSuperviewEdges];
    wallpaperBackgroundView.backgroundColor = Theme.backgroundColor;
    wallpaperBackgroundView.userInteractionEnabled = false;
    self.wallpaperBackgroundView = wallpaperBackgroundView;
    
    [self updateWallPaper];
    
    [self applyTheme];
    [self.conversationViewModel viewDidLoad];
//    self.wallpaperBackgroundView
...
- (void)applicationDidBecomeActive:(NSNotification *)notification
{
    [self startReadTimer];
    if ([self.thread isKindOfClass:TSGroupThread.class]) {
        //Check group call status
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [AppEnvironment.shared.callService checkCallInProgress:(id)self.thread completion:^(BOOL isInProgress) {
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    UINavigationController *rootNav = SignalApp.sharedApp.rootNav;
    if (self.navigationController &&
        rootNav && self.navigationController == rootNav) {
    ...
    else if ([self.thread isKindOfClass:TSContactThread.class]) {
        TSContactThread *contactThread = (id)self.thread;
        [OWSProfileManager.sharedManager fetchProfileForAddress:contactThread.contactAddress];
    }
}
...
//    [self.collectionView.collectionViewLayout invalidateLayout];
    ...
    [self updatePinedMessagesView];
//    NSDate *date = [NSDate new];
//    NSString *testMessage = [NSString stringWithFormat:@"Hoang Test Messsage %.0f", date.timeIntervalSince1970];
//    [self sendTestMessage:testMessage];
}
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [self.expandPinnedMessageView removeFromSuperview];
    self.expandPinnedMessageView = nil;
}
...
- (void)updateBarButtonItems
{
    ...
    switch (self.uiMode) {
        ...
        case ConversationUIMode_Normal: {
            ...
            if ([self canCall]) {
                if ([self.thread isKindOfClass:TSGroupThread.class] &&
                    ((TSGroupThread*)self.thread).hasCallInProgress) {
                    UIBarButtonItem *joinButtonItem = [self createJoinCallButton];
                    [barButtons addObject:joinButtonItem];
                }else {
                    // We use UIButtons with [UIBarButtonItem initWithCustomView:...] instead of
                    // UIBarButtonItem in order to ensure that these buttons are spaced tightly.
                    // The contents of the navigation bar are cramped in this view.
                    UIBarButtonItem *callButtonItem = [self createAudioCallButton];
                    [barButtons addObject:callButtonItem];
                    
                    UIBarButtonItem *calleoVidButtonItem = [self createVideoCallButton];
                    [barButtons addObject:calleoVidButtonItem];
                }
            }
...
- (void)updateWallPaper {
    void(^changeBackgroundWallpaper)(id) = ^(NSString *filePath) {
        UIImage *ima = nil;
        if ([filePath isKindOfClass:NSString.class] &&
            ...
            if (data) {
                ...
                if (base64Str) {
                    NSData *decodedData = [[NSData alloc] initWithBase64EncodedString:base64Str options:NSDataBase64DecodingIgnoreUnknownCharacters];
                    if (decodedData) {
                        ima = [UIImage imageWithData:decodedData];
                    }
                }
                if (!ima) {
        ...
        self.wallpaperBackgroundView.image = ima;
    };
    NSUserDefaults *userDefaults = [[NSUserDefaults alloc] initWithSuiteName:TSConstants.applicationGroup];
    if ([self.thread isMemberOfClass:TSContactThread.class]) {
        NSString *filePath = nil;
        TSContactThread *contactThread = (id)self.thread;
        NSString *phoneNumber = contactThread.contactPhoneNumber;
        NSString *key = [NSString stringWithFormat:@"CHAT_WALLPAPER_\%@", phoneNumber];
        NSString *fileName = [userDefaults objectForKey:key];
        if ([fileName isKindOfClass:NSString.class] &&
            fileName.length > 0) {
            filePath = [OWSFileSystem.appDocumentDirectoryPath stringByAppendingPathComponent:key];
        }
        changeBackgroundWallpaper(filePath);
    }
    else if ([self.thread isMemberOfClass:TSGroupThread.class]) {
        NSString *filePath = nil;
        TSGroupThread *groupThread = (id)self.thread;
...
- (void)startVideoCall
{
//    NSInteger start = self.thread.lastInteractionRowId;
//    [self test_SendMessasge:start number:start + 50];
...
{
    if (![self.thread isKindOfClass:[TSContactThread class]] &&
        ![self.thread isKindOfClass:[TSGroupThread class]]) {
    ...
    [self ows_askForMicrophonePermissions:^(BOOL granted) {
        if (granted) {
            BOOL success = [AppEnvironment.shared.callService startCall:weakSelf.thread audioOnly:!isVideo fromVC:weakSelf];
            if (success) {
                //Hide the pinnedMessageView
                self.expandPinnedMessageView.hidden = true;
            }
        } else {
            OWSLogInfo(@"we do not have microphone permission.");
            [weakSelf ows_showNoMicrophonePermissionActionSheet];
        }
    }];
}
...
- (void)joinGroupCall {
    ...
    BOOL success = [AppEnvironment.shared.callService joinCall:groupThread callId:groupThread.callIdInProgress audioOnly:true fromVC:self];
    if (success) {
        //Hide the pinnedMessageView
        self.expandPinnedMessageView.hidden = true;
    }
...
- (BOOL)canCall
{
    ...
    if ([contactThread isKindOfClass:TSContactThread.class] &&
        contactThread.isNoteToSelf) {
        return NO;
    }
    if ([contactThread isKindOfClass:TSContactThread.class]) {
        //Does not allow make call if the receiver does not accept friend yet
        __block BOOL isFriend = false;
        for (SignalServiceAddress *address in self.thread.recipientAddresses) {
            [self.databaseStorage readWithBlock:^(SDSAnyReadTransaction * transaction) {
                NSString *profileName = [OWSProfileManager.sharedManager fullNameForAddress:address transaction:transaction];
...
- (UIBarButtonItem*)createAudioCallButton {
    const CGFloat kBarButtonSize = 44;
    // We use UIButtons with [UIBarButtonItem initWithCustomView:...] instead of
    // UIBarButtonItem in order to ensure that these buttons are spaced tightly.
    // The contents of the navigation bar are cramped in this view.
    UIButton *callButton = [UIButton buttonWithType:UIButtonTypeCustom];
    UIImage *image = [UIImage imageNamed:@"ic-audio-call-new"];
    ...
    callButton.accessibilityLabel
        = NSLocalizedString(@"CALL_AUDIO_LABEL", "Accessibility label for placing call button");
...
- (UIBarButtonItem*)createVideoCallButton {
    const CGFloat kBarButtonSize = 44;
    // We use UIButtons with [UIBarButtonItem initWithCustomView:...] instead of
    // UIBarButtonItem in order to ensure that these buttons are spaced tightly.
    // The contents of the navigation bar are cramped in this view.
    UIButton *callButton = [UIButton buttonWithType:UIButtonTypeCustom];
    UIImage *image = [UIImage imageNamed:@"ic-video-call-new"];
    [callButton setImage:image forState:UIControlStateNormal];
    ...
    callButton.imageEdgeInsets = imageEdgeInsets;
    callButton.accessibilityLabel
        = NSLocalizedString(@"CALL_VIDEO_LABEL", "Accessibility label for placing call button");
    [callButton addTarget:self
...
- (UIBarButtonItem*)createJoinCallButton {
//    const CGFloat kBarButtonSize = 44;
    // We use UIButtons with [UIBarButtonItem initWithCustomView:...] instead of
    // UIBarButtonItem in order to ensure that these buttons are spaced tightly.
    // The contents of the navigation bar are cramped in this view.
    UIButton *joinCallButton = [UIButton buttonWithType:UIButtonTypeCustom];
    
    if (OWSWindowManager.sharedManager.hasCall) {
        joinCallButton.enabled = NO;
        joinCallButton.userInteractionEnabled = NO;
        joinCallButton.alpha = 0.5;
    } else {
        joinCallButton.enabled = YES;
        joinCallButton.userInteractionEnabled = YES;
        joinCallButton.alpha = 1;
    }
    joinCallButton.backgroundColor = UIColor.whiteColor;
    [joinCallButton setTitle:NSLocalizedString(@"JOIN_CALL", @"Accessibility label for join call button") forState:UIControlStateNormal];
    joinCallButton.titleLabel.font = [UIFont ows_semiboldFontWithSize:12.f];
    [joinCallButton setTitleColor:Theme.blueTextColor forState:UIControlStateNormal];
    joinCallButton.layer.cornerRadius = 15;
    joinCallButton.layer.masksToBounds = true;
    
    joinCallButton.accessibilityLabel
        = NSLocalizedString(@"JOIN_CALL_LABEL", "Accessibility label for join call button");
    [joinCallButton addTarget:self
                   action:@selector(joinGroupCall)
         forControlEvents:UIControlEventTouchUpInside];
    joinCallButton.frame = CGRectMake(0, 0, 80, 30);
    
    UIBarButtonItem *barButtonItem = [[UIBarButtonItem alloc]
                                      initWithCustomView:joinCallButton
                                      accessibilityIdentifier:ACCESSIBILITY_IDENTIFIER_WITH_NAME(self, @"call")];
    return barButtonItem;
}
...
- (void)showConversationSettingsAndShowVerification:(BOOL)showVerification
{
    ...
    self.wallpaperBackgroundView.hidden = true;
    self.view.backgroundColor = UIColor.clearColor;
    self.inputToolbar.hidden = true;
    
    UIGraphicsBeginImageContextWithOptions(self.view.frameSize,false,0.0f);
    CGContextRef context = UIGraphicsGetCurrentContext();
    [self.view.layer renderInContext:context];
    UIImage *capturedImage = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    NSData *data = UIImagePNGRepresentation(capturedImage);
    NSString *filePath = [OWSFileSystem.appDocumentDirectoryPath stringByAppendingPathComponent:@"CURRENT_CONVERSATION_SNAPSHOT"];
    [data writeToFile:filePath atomically:true];
    
    self.wallpaperBackgroundView.hidden = false;
...
- (void)handleCallTap:(TSCall *)call
{
    if ([self.thread isKindOfClass:TSGroupThread.class]) {
        TSGroupThread *groupThread = (id)self.thread;
        if (groupThread.hasCallInProgress) {
            [self joinGroupCall];
            return;
        }
    }
    ...
    if ([self.thread isKindOfClass:TSContactThread.class]) {
        TSContactThread *contactThread = (TSContactThread *)self.thread;
        displayName = [self.contactsManager displayNameForAddress:contactThread.contactAddress];
    }
    else if ([self.thread isKindOfClass:TSGroupThread.class]) {
        [self.databaseStorage uiReadWithBlock:^(SDSAnyReadTransaction * _Nonnull trans) {
            displayName = [self.contactsManager displayNameForThread:self.thread transaction:trans];
        }];
    }
                 ...
                 withNewNameComponents:(NSPersonNameComponents *)newNameComponents
{
    ...
    OWSNavigationController *nav = [[OWSNavigationController alloc] initWithRootViewController:contactViewController];
...
{
    TSInfoMessage *infoMsg = (id)viewItem.interaction;
    //Don't allow action for pin and group call
    if ([infoMsg isKindOfClass:TSInfoMessage.class] &&
        (infoMsg.messageType == TSInfoMessagePinMessage ||
         infoMsg.messageType == TSInfoMessageGroupCall ||
         infoMsg.messageType == TSInfoMessageSetWallPaper)) {
        return;
    }
    NSArray<MessageAction *> *messageActions =
...
- (void)applyTheme
{
    ...
    self.wallpaperBackgroundView.backgroundColor = Theme.backgroundColor;
...
- (void)conversationViewModelDidUpdateWithSneakyTransaction:(ConversationUpdate *)conversationUpdate
{
    ...
    if (self.threadViewModel.isGroupThread &&
        self.view.window) {
        [self updatePinedMessagesView];
        
        //In case user left group but another in group add this user again
        if (!self.userLeftGroup &&
            self.inputToolbar.hidden) {
            [self updateInputVisibility];
        }
    }
}
...
- (void)updateContentInsetsAnimated:(BOOL)animated
{
    ...
    newInsets.bottom += self.view.safeAreaInsets.bottom;
