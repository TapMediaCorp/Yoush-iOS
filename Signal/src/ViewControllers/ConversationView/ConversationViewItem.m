...
{
    TSMessage *message = (id)self.interaction;
...
- (void)deleteAction
{
    DatabaseStorageAsyncWrite(self.databaseStorage, ^(SDSAnyWriteTransaction *transaction) {
        //Find pin message for this interaction
        NSMutableArray *pinMessages = @[].mutableCopy;
        if ([self.interaction isKindOfClass:TSIncomingMessage.class] ||
            [self.interaction isKindOfClass:TSOutgoingMessage.class]) {
            NSArray *interactions = [InteractionFinder fetchPinInfoMessage:self.thread.uniqueId transaction:transaction];
            for (TSInfoMessage *infoObj in interactions) {
                ...
                NSString *messageId = userInfo[InfoMessageUserInfoPinMessageId];
                if ([messageId isEqualToString:self.interaction.uniqueId]) {
                    [pinMessages addObject:infoObj];
                }
            }
        }
        if (pinMessages.count > 0) {
            NSLog(@"Pin message should remove %@", pinMessages);
        }
        
        //Remove pin message belong to this interaction
        for (TSInfoMessage *infoObj in pinMessages) {
            [infoObj anyRemoveWithTransaction:transaction];
        }
