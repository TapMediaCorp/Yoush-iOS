...
@objc(OWSPinSetupConvViewController)
public class PinSetupConvViewController: OWSViewController {
    ...
    @objc
    public var isSetupHiddenConversationPin = false
    ...
    enum Mode {
        ...
        var isCreating: Bool {
            guard case .creating = self else {return false}
            return true
        }
    }
    private var mode: Mode

    private var initialMode: Mode
    ...
    private let completionHandler: (PinSetupConvViewController, Error?) -> Void
    ...
    init(
        ...
        completionHandler: @escaping (PinSetupConvViewController, Error?) -> Void
    ) {
    ...
    @objc
    class func creatingRegistrationLock(completionHandler: @escaping (PinSetupConvViewController, Error?) -> Void) -> PinSetupConvViewController {
        return .init(mode: .creating, enableRegistrationLock: true, completionHandler: completionHandler)
    }

    @objc
    class func creating(pinCode: String, completionHandler: @escaping (PinSetupConvViewController, Error?) -> Void) -> PinSetupConvViewController {
        if(!pinCode.isEmpty) {
            return .init(mode: .confirming(pinToMatch: pinCode), completionHandler: completionHandler)
        }
        return .init(mode: .creating, completionHandler: completionHandler)
    }

    @objc
    class func changing(completionHandler: @escaping (PinSetupConvViewController, Error?) -> Void) -> PinSetupConvViewController {
...
//        CONFIRM
        ...
        // We have no nav bar and build our own back button + title label
            ...
            if mode.isConfirming  || isSetupHiddenConversationPin {
            ...
            let label = UILabel()
            label.textColor = UIColor(rgbHex: 0x707070)
            label.font = UIFont(name: Fonts.Roboto_Regular, size: 25)
        ...
        explanationLabel.textColor = UIColor(rgbHex: 0x707070)
        explanationLabel.font = UIFont(name: Fonts.Roboto_Regular, size: 14)
        ...
        case .creating:
        // CREAD PIN
            if isSetupHiddenConversationPin {
                explanationText = NSLocalizedString("HIDDEN_CONVERSATION_PIN_CREATION_EXPLANATION",
                                                    comment: "")
            }else {
...
//            )
        ...
        validationWarningLabel.textColor = UIColor(rgbHex: 0x707070)
        validationWarningLabel.textAlignment = .center
        validationWarningLabel.font = UIFont(name: Fonts.Roboto_Regular, size: 12)
        ...
        pinTypeToggle.titleLabel?.font = UIFont(name: Fonts.Roboto_Regular, size: 16)
        ...
        nextButton.setTitle(title: CommonStrings.nextButton, font:  UIFont(name: Fonts.Roboto_Medium, size: 20)!, titleColor: .white)
        nextButton.setBackgroundColors(upColor: Theme.youshGoldColor)
    ...
    private func tryToContinue() {
        ...
        case .creating, .changing, .recreating:
            let confirmingVC = PinSetupConvViewController(
    ...
    private func updatePinType() {
        ...
        switch pinType {
        case .numeric:
//            pinTypeToggle.setTitle(NSLocalizedString("",
...
//                                                     comment: "Button asking if the user would like to create an alphanumeric PIN"), for: .normal)
    ...
    private func enable2FAAndContinue(withPin pin: String) {
        ...
        self.databaseStorage.write { transaction in
            return self.keyValueStore.setString(pin, key: "PIN_CODE", transaction: transaction)
        }
        if case .confirming(let _) = mode,
           isSetupHiddenConversationPin == true {
            //Should handle at completionHandler
...
extension PinSetupConvViewController: UITextFieldDelegate {
...
extension PinSetupConvViewController {
