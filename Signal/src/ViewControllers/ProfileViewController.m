...
- (void)loadView
{
    ...
    // headlinelabel.font = [UIFont fontWithName:@"Roboto-Medium" size: 20];
...
- (void)createViews
{
    ...
    [self.cameraImageView setTemplateImageName:@"31-icon-camera" tintColor:Theme.grayBorderIconColor];
    [self.cameraImageView autoSetDimensionsToSize:CGSizeMake(28, 28)];
    self.cameraImageView.contentMode = UIViewContentModeCenter;
    self.cameraImageView.backgroundColor = Theme.toastForegroundColor;
    self.cameraImageView.layer.cornerRadius = 14;
...
//    self.profileNamePreviewLabel.textColor = [UIColor colorWithRed:112.0f/255.0f
//                                                        green:112.0f/255.0f
//                                                         blue:112.0f/255.0f
//                                                        alpha:1.0f];
    self.profileNamePreviewLabel.textColor = Theme.blueTextColor;
    self.profileNamePreviewLabel.font = [UIFont fontWithName:@"Roboto-Medium" size:16];
    ...
    void (^addGivenNameRow)(void) = ^{
        ...
        givenNameLabel.textColor = [UIColor colorWithRed:112.0f/255.0f
                                        green:112.0f/255.0f
                                         blue:112.0f/255.0f
                                        alpha:1.0f];
        givenNameLabel.font = [UIFont fontWithName:@"Roboto-Medium" size:16];
        ...
        givenNameTextField.textColor = Theme.blueTextColor;
        givenNameTextField.font = [UIFont fontWithName:@"Roboto-Medium" size:16];
    ...
    void (^addFamilyNameRow)(void) = ^{
        ...
        familyNameLabel.textColor = [UIColor colorWithRed:112.0f/255.0f
                                        green:112.0f/255.0f
                                         blue:112.0f/255.0f
                                        alpha:1.0f];
        familyNameLabel.font = [UIFont fontWithName:@"Roboto-Medium" size:16];
        ...
        familyNameTextField.textColor = Theme.blueTextColor;
        familyNameTextField.font = [UIFont fontWithName:@"Roboto-Medium" size:16];
        ...
        case ProfileViewMode_ExperienceUpgrade: {
            ...
            OWSFlatButton *saveButton =
                [OWSFlatButton buttonWithTitle:NSLocalizedString(@"PROFILE_VIEW_SAVE_BUTTON",
                                          ...
                                          font:[UIFont fontWithName:@"Roboto-Medium" size: 20]
                                    titleColor:[UIColor whiteColor]
                               backgroundColor:Theme.youshGoldColor
                                        target:self
...
{
    NSString *nText = [textField.text stringByReplacingCharactersInRange:editingRange withString:insertionText];
    if (![self isValidName:nText]) {
        return false;
    }
...
//            [[[OWSContactAvatarBuilder alloc] initForLocalUserWithDiameter:self.avatarSize] buildDefaultImage];
        ...
        self.avatarView.image =  [UIImage imageNamed:@"30-avatar-profile.png"];
    }
}
...
- (void)infoRowTapped:(UIGestureRecognizer *)sender
{
    if (sender.state == UIGestureRecognizerStateRecognized) {
        SFSafariViewController *safariVC = [[SFSafariViewController alloc]
            initWithURL:[NSURL URLWithString:@"https://support.tapofthink.com/hc/articles/360007459591"]];
...
- (BOOL)isValidName:(NSString*)nameStr {
    ...
    if ([nameStr.lowercaseString rangeOfCharacterFromSet:specialCharacterSet].length) {
        NSLog(@"contains special characters");
        return false;
    }
    
    if ([nameStr containsEmoji]) {
        NSLog(@"Contains Emoji");
        return false;
    }
    return true;
}
