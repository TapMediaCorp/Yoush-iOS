...
@objc class CallAudioService: NSObject, CallObserver {
    ...
    private var forceRinging = false
...
//            assert(!Thread.isMainThread)
    ...
    public func ensureProperAudioSession(call: SignalCall?) {
        AssertIsOnMainThread()
    ...
    func handleRemoteRinging(call: SignalCall) {
        AssertIsOnMainThread()
        Logger.debug("")
    ...
    func handleLocalRinging(call: SignalCall) {
        AssertIsOnMainThread()
        Logger.debug("")
    ...
    func startRinging(call: SignalCall, forceRing:Bool = false) {
        forceRinging = forceRing
        
        guard handleRinging || forceRinging else {
