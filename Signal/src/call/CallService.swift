...
protocol CallServiceObserver: NSObject {
    /**
...
@objc final public class CallService: NSObject, CallObserver {
    ...
    internal var calls: Set<SignalCall> = [] {
        didSet {
            AssertIsOnMainThread()
        }
    }
...
//        callManager.delegate = self
    ...
    @objc
    public var isRTCAudioSessionEnabled: Bool {
        get {
            return rtcAudioSession.isAudioEnabled
        }
        set {
            print("Updating rtc audio session status \(newValue)")
    ...
    func handleOutgoingCall(_ call: SignalCall) {
        ...
        let callRecord = TSCall(callType: .outgoingIncomplete, thread: thread, sentAtTimestamp: call.sentAtTimestamp)
...
//        }
    ...
    public func handleAcceptCall(_ call: SignalCall) {
        ...
        guard let thread = call.currentThread else {
            handleFailedCall(failedCall: call, error: OWSAssertionError("no thread for call: \(call)"))
            return
        }

        let callRecord = TSCall(callType: .incomingIncomplete, thread: thread, sentAtTimestamp: call.sentAtTimestamp)
        databaseStorage.write { transaction in
            callRecord.anyInsert(transaction: transaction)
        }
        call.callRecord = callRecord
...
//    }
    ...
    private func handleRinging(call: SignalCall) {
        ...
        default :
            Logger.info("Unhandle \(#function): \(call).")
        }
    }
...
//    }
    ...
    func placeCall(_ call: SignalCall)
    {
        calls.insert(call)
        currentCall = call
    }
    
    func terminateCall(_ signalCall: SignalCall?) {
        guard let call = signalCall else {
            return
        }
        if calls.remove(call) == nil {
        ...
        if !hasCallInProgress {
            self.isRTCAudioSessionEnabled = false
        }
        ...
        if currentCall != nil {
            if currentCall!.callId == call.callId {
                ...
                print("terminating another call: \(call)")
            }
        }
    }
    
    func handleIncomingJitsiCallMessage(_ thread:TSThread, data: [String: Any]) {
        //If there are many differences between group call and one-to-one call, we should seperate them
        //for now, just use group call logic to handle one-to-one call
        handleIncomingJitsiGroupCallMessage(thread, data: data)
    }
    
    func findCall(_ localCallId: UUID) -> SignalCall? {
        guard let index = calls.firstIndex(where: { $0.localId == localCallId }) else {
            return nil
        }
        return calls[index]
    }
