...
//        self.callService.handleReceivedBusy(thread: thread, callId: busy.id, sourceDevice: sourceDevice)
    ...
    public func receivedJitsiGroupCall(_ thread: TSThread, data: Any) {
        guard let callData = data as? [String:Any],
              let _ = thread as? TSGroupThread else {
            return
        }
        callService.handleIncomingJitsiGroupCallMessage(thread, data: callData)
    }

    public func receivedJitsiCall(_ thread: TSThread, data: Any) {
        guard let callData = data as? [String:Any],
              let _ = thread as? TSContactThread else {
            return
        }
        callService.handleIncomingJitsiCallMessage(thread, data: callData)
    }
}
