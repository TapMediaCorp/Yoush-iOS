...
@property (strong, nonatomic) NSUserDefaults *appGroupUserDefaults;
...
- (id)init
{
    ...
    _appGroupUserDefaults = [[NSUserDefaults alloc] initWithSuiteName:TSConstants.applicationGroup];
    return self;
}
...
- (NSString *)displayNameForThread:(TSThread *)thread transaction:(SDSAnyReadTransaction *)transaction
{
    if (thread.isNoteToSelf) {
        ...
        if (contactThread.nameAlias.length > 0) {
            return contactThread.nameAlias;
        }
...
- (NSString *)displayNameForAddress:(SignalServiceAddress *)address transaction:(SDSAnyReadTransaction *)transaction
{
    ...
    NSString *aliasKey = [NSString stringWithFormat:@"NAME_ALIAS_%@", address.phoneNumber];
    NSString *aliasName = [self.appGroupUserDefaults objectForKey:aliasKey];
