...
public class OWSAudioSession: NSObject {
    ...
    @objc
    @discardableResult public func startAudioActivity(_ audioActivity: AudioActivity) -> Bool {
    ...
    @objc
    public func containsAudioActivity(_ audioActivity: AudioActivity) -> Bool {
        objc_sync_enter(self)
        defer { objc_sync_exit(self) }

        if let _ = currentActivities.first(where: {$0.value == audioActivity}) {
            return true
        }
        return false
    }
    ...
    func reconcileAudioCategory() throws {
        ...
        var managedByWebRTC = false
        print("HoangHo: Current aggregateBehaviors \(aggregateBehaviors)")
        if aggregateBehaviors.contains(.call) {
            ...
            // session handling.
            managedByWebRTC = true
            print("HoangHo: Current audio session managed by webRTC \(avAudioSession.category) - \(avAudioSession.mode) - \(avAudioSession.categoryOptions)")
        ...
        if managedByWebRTC == false {
            print("HoangHo: Current audio session category \(avAudioSession.category) - \(avAudioSession.mode) - \(avAudioSession.categoryOptions)")
        }
    }
...
//    }
    @objc
    public var isRTCAudioEnabled = true
}
