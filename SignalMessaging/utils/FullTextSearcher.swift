...
public class HideConversationSearchResult<SortKey>: Comparable where SortKey: Comparable {
    public let thread: ThreadViewModel
    ...
    public static func < (lhs: HideConversationSearchResult, rhs: HideConversationSearchResult) -> Bool {
        return lhs.sortKey < rhs.sortKey
    }
...
public class HomeScreenSearchResultSet: NSObject {
    ...
    // hide conversations
    public let hideConversations: [HideConversationSearchResult<ConversationSortKey>]
    ...
    public init(searchText: String, hideConversations: [HideConversationSearchResult<ConversationSortKey>], conversations: [ConversationSearchResult<ConversationSortKey>], contacts: [ContactSearchResult], messages: [ConversationSearchResult<MessageSortKey>]) {
        self.searchText = searchText
        // hide conversations
        self.hideConversations = hideConversations
    ...
    public class var empty: HomeScreenSearchResultSet {
        // hide conversations
        return HomeScreenSearchResultSet(searchText: "", hideConversations: [], conversations: [], contacts: [], messages: [])
    }

    public var isEmpty: Bool {
        // hide conversations
        return hideConversations.isEmpty && conversations.isEmpty && contacts.isEmpty && messages.isEmpty
    }
}
...
public class FullTextSearcher: NSObject {
    ...
    @objc public let keyValueStore = SDSKeyValueStore(collection: "PIN_CODE")
    ...
    public func searchForHomeScreen(searchText: String,
        ...
        // hide conversations
        var hideConversations: [HideConversationSearchResult<ConversationSortKey>] = []
        ...
        var globalPinCode = ""
        if let pinCode = CurrentAppContext().hideConversationPinCode,
           pinCode.count > 0 {
            globalPinCode = pinCode
        }
        
        var count: UInt = 0
        self.finder.enumerateObjects(pindCode: globalPinCode, searchText: searchText, transaction: transaction) { (match: Any, snippet: String?, stop: UnsafeMutablePointer<ObjCBool>) in
            ...
            if let thread = match as? TSThread {
                ...
                let sortKey = ConversationSortKey(isContactThread: thread is TSContactThread,
                 ...
                 let searchResultHideConversations = HideConversationSearchResult(thread: threadViewModel, sortKey: sortKey)
                switch thread {
                case is TSGroupThread:
                    // hide conversations
                    if thread.isHided,
                       globalPinCode == searchText {
                        hideConversations.append(searchResultHideConversations)
                    }else {
        ...
        // The contact results are pre-sorted by display name.
        // hide conversations
        hideConversations.sort(by: >)
        conversations.sort(by: >)
        messages.sort(by: >)
        // Order "other" contact results by display name.
        otherContacts.sort()

        return HomeScreenSearchResultSet(searchText: searchText, hideConversations: hideConversations, conversations: conversations, contacts: otherContacts, messages: messages)
    }

    @objc
    public func searchAllHiddenConversation(transaction: SDSAnyReadTransaction) -> [TSThread] {
        var hideThreads = [TSThread]()
        
        let maxResults: UInt = 100000
            
        var count: UInt = 0
        self.finder.fetchAllHiddenThread(transaction: transaction) { (match: Any, snippet: String?, stop: UnsafeMutablePointer<ObjCBool>) in
            count += 1
            guard count < maxResults else {
                stop.pointee = true
                return
