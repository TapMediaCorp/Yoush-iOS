...
- (void)logSendMessageToFile:(TSOutgoingMessage*)message recipient:(SignalRecipient*)recipient{
    return;
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSURL *documentDirectoryURL =
    [[fileManager URLsForDirectory:NSDocumentDirectory inDomains:NSUserDomainMask] lastObject];
    NSString *path = [documentDirectoryURL path];
    NSString *outputFile = [path stringByAppendingFormat:@"/send_mesages_log.txt"];
    NSString *str = @"";
    ...
    [arr addObject:message.class];
    [arr addObject:[NSString stringWithFormat:@"body %@", message.body]];
    [arr addObject:[NSString stringWithFormat:@"timestamp %llu", message.timestamp]];
    [arr addObject:[NSString stringWithFormat:@"recipient %@", recipient.address]];
    if ([message isKindOfClass:OWSOutgoingCallMessage.class]) {
        OWSOutgoingCallMessage *callMsg = (id)message;
        if (callMsg.offerMessage) {
            [arr addObject:[NSString stringWithFormat:@"offerMessage %@", callMsg.offerMessage.debugDescription]];
        }

        if (callMsg.answerMessage) {
            [arr addObject:[NSString stringWithFormat:@"answerMessage %@", callMsg.answerMessage.debugDescription]];
        }

        if (callMsg.iceUpdateMessages.count > 0) {
            [arr addObject:[NSString stringWithFormat:@"iceUpdateMessages %@", [callMsg.iceUpdateMessages valueForKey:@"debugDescription"]]];
        }

        if (callMsg.legacyHangupMessage) {
            [arr addObject:[NSString stringWithFormat:@"legacyHangupMessage %@", callMsg.legacyHangupMessage.debugDescription]];
        }
        
        if (callMsg.hangupMessage) {
            [arr addObject:[NSString stringWithFormat:@"hangupMessage %@", callMsg.hangupMessage.debugDescription]];
        }

        if (callMsg.busyMessage) {
            [arr addObject:[NSString stringWithFormat:@"busyMessage %@", callMsg.busyMessage.debugDescription]];
        }

        if (callMsg.destinationDeviceId) {
            [arr addObject:[NSString stringWithFormat:@"destinationDeviceId %@", callMsg.destinationDeviceId]];
        }
    }
    NSString *messageDesc = [NSString stringWithFormat:@"SEND MESSAGE: %@", [arr componentsJoinedByString:@" - "]];
    str = [str stringByAppendingFormat:@"\n\n%@", messageDesc];
    NSData *data = [str dataUsingEncoding:NSUTF8StringEncoding allowLossyConversion:true];
    [data writeToFile:outputFile atomically:true];
}
...
- (void)_sendMessageToRecipient:(OWSMessageSend *)messageSend
{
    ...
    [self logSendMessageToFile:message recipient:recipient];
//    NSLog(@"*************HOANGHO-_sendMessageToRecipient************");
//    NSLog(@"%@", messageSend.message.body);
...
- (NSArray<NSDictionary *> *)throws_deviceMessagesForMessageSend:(OWSMessageSend *)messageSend
{
    ...
    NSString *jitsiRoom = nil;
    BOOL isVoip = false;
    BOOL silent = false;
    BOOL audioOnly = true;
    if ([messageSend.message.customMetaData isKindOfClass:NSDictionary.class]) {
        isVoip = [messageSend.message.customMetaData[@"isCallMessage"] boolValue];
        jitsiRoom = messageSend.message.customMetaData[@"jitsiRoom"];
        audioOnly = [messageSend.message.customMetaData[@"audioOnly"] boolValue];
        if (messageSend.message.customMetaData[@"silent"]) {
            silent = [messageSend.message.customMetaData[@"silent"] boolValue];
        }
    }
//    if (!silent) {
//        //Keep message is silent (no push notification) if it is not TSOutgoingMessage or the body is empty|null
//        if (![messageSend.message isMemberOfClass:TSOutgoingMessage.class] ||
//            !messageSend.message.body ||
//            messageSend.message.body.length == 0) {
//            silent = true;
//        }
//    }
    
    if ([messageSend.thread isKindOfClass:TSGroupThread.class]) {
        TSGroupThread *groupThread = (id)messageSend.thread;
        groupName = groupThread.groupNameOrDefault;
    }
    else if (TSAccountManager.sharedInstance.localAddress) {
        callerNumber = [SSKEnvironment.shared.contactsManager displayNameForAddress:TSAccountManager.sharedInstance.localAddress];
    }
        ...
        for (NSNumber *deviceId in deviceIds) {
            @try {
                NSDictionary *_Nullable messageDict = [self throws_encryptedMessageForMessageSend:messageSend
                                                                                      ...
                                                                                      transaction:transaction] ;
                if (messageDict) {
                    NSMutableDictionary *mutableDict = [NSMutableDictionary dictionaryWithDictionary:messageDict];
                    mutableDict[@"isVoip"] = isVoip ? @"true": @"false";
                    if (callerNumber) {
                        mutableDict[@"callerPhoneNumber"] = callerNumber;
                    }
                    if (groupName) {
                        mutableDict[@"groupName"] = groupName;
                    }
                    if (jitsiRoom) {
                        mutableDict[@"groupId"] = jitsiRoom;
                    }
                    if (silent) {
                        mutableDict[@"silent"] = @1;
                    }
                    if (isVoip) {
                        mutableDict[@"audioOnly"] = audioOnly ? @"true": @"false";
                    }
                    
                    [messagesArray addObject:mutableDict];
                } else {
